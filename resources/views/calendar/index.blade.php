        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white dark:bg-gray-700 shadow rounded-lg p-4">
                <form action="{{ route('calendar.index') }}" method="GET" class="flex items-center gap-4">
                    <x-input-label for="year" value="{{ __('calendar.form.year_label') }}" />
                    <x-text-input id="year" name="year" type="number" :value="$year" min="2000" max="2100"
                        class="border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                    <x-primary-button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @for ($m = 1; $m <= 12; $m++)
                    @php
                        ...
                        // Obtenemos el nombre del mes en español (aseg\u00FArate de haber configurado Carbon en español)
                        $monthName = ucfirst($monthDate->translatedFormat('F'));
                        $daysInMonth = $monthDate->daysInMonth;
                        // dayOfWeekIso devuelve 1 para lunes y 7 para domingo; lo usamos para calcular el offset de celdas vac\u00EDas
                    ...
                    <div class="bg-white dark:bg-gray-700 shadow rounded-lg p-4">
                        ...
                        <table class="w-full text-center border-collapse text-xs">
                            ...
                            <tbody>
                                @php
                                    // Calcula cuántas celdas totales (completas semanas) se deben mostrar
                                ...
                                @for ($i = 0; $i < $totalCells; $i++)
                                    ...
                                    @else
                                        @php
                                            // Formato de la fecha para buscar en el agrupamiento
                                            $currentDate = \Carbon\Carbon::createFromDate($year, $m, $day)->format('Y-m-d');
                                            // Recupera los registros asignados a ese d\u00EDa o una colecci\u00F3n vac\u00EDa
